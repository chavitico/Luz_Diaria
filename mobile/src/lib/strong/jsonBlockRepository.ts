...
// Pre-built inverted index: strongId → sorted verse labels (e.g. "Juan 20:22")
// Generated by scripts/generate-strong-occurrences.ts from alignment data.
let _occurrenceLabels: Record<string, string[]> | null = null;
function getOccurrenceLabels(): Record<string, string[]> {
  if (!_occurrenceLabels) {
    _occurrenceLabels = require('./data/strong_occurrence_index.json') as Record<string, string[]>;
  }
  return _occurrenceLabels;
}
import {
  normalizeEs,
  SPANISH_GLOSS_INDEX,
...
export class JsonBlockStrongRepository implements IStrongRepository {
  ...
  getRelatedVerses(strongId: string): string[] {
    return getOccurrenceLabels()[strongId] ?? [];
  }
